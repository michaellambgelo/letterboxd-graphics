...
export async function newestArchiveDir(root) {
...
// Film-level Letterboxd URIs (boxd.it short links) from the newest archive's
// watched.csv. These are exact — the URI is the film Michael actually logged —
// so they beat a title+year search for poster lookup: /film/luca-2021 resolves
// itself, with no same-title collision to disambiguate.
export async function loadFilmUris() {
  const root = viewerPath();
  const archive = await newestArchiveDir(root);
  const map = new Map();
  ...
  let rows;
  try {
    rows = parseCsv(await readFile(joinPath(archive, 'watched.csv'), 'utf8'));
  } catch {
    return map;
  }
  for (const row of rows) {
    const uri = row['Letterboxd URI'];
    if (row.Name && row.Year && uri) map.set(filmKey(row.Name, row.Year), uri);
  }
  return map;
}
