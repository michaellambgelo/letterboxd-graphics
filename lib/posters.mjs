...
// Letterboxd's image CDN renders sizes on demand: the `0-<w>-0-<h>-crop` segment
// of a resized URL is a *request*, not a fixed asset — verified against the CDN,
// 0-600-0-900 (50 KB) and 0-1000-0-1500 (154 KB) both return 200 for the same
// image. The film page's JSON-LD offers 600x900, but a poster renders ~434 device
// px wide at 2x export scale, leaving almost no headroom. Ask for more.
//
// Borrowed from boxd-card/src/shared/posters.ts, which documents the same trick
// and uses it for its canvas cards.
const LB_TARGET_W = 1000;
const LB_TARGET_H = 1500;

// Poster aspect is 2:3. Avatars (1:1) and backdrops (16:9) travel through the
// same URL shape and must keep their own dimensions, so match on ratio and leave
// anything else alone.
const MIN_POSTER_RATIO = 1.4;
const MAX_POSTER_RATIO = 1.6;

export function upscaleLetterboxdPoster(url) {
  if (typeof url !== 'string' || !url.includes('a.ltrbxd.com/resized/')) return url;
  return url.replace(/-0-(\d+)-0-(\d+)-crop\./, (whole, w, h) => {
    const ratio = Number(h) / Number(w);
    if (!(ratio >= MIN_POSTER_RATIO && ratio <= MAX_POSTER_RATIO)) return whole;
    if (Number(w) >= LB_TARGET_W) return whole;   // already as large or larger
    return `-0-${LB_TARGET_W}-0-${LB_TARGET_H}-crop.`;
  });
}
...
export async function letterboxdFilmInfo(uri) {
  ...
  poster = upscaleLetterboxdPoster(poster);
...
export const TMDB_HELP =
  'No TMDB credential stored. Alternate poster art needs one — either:\n' +
  '  export TMDB_API_KEY=...\n' +
  'or, to keep it out of your shell history:\n' +
  '  security add-generic-password -a "$USER" -s dotfiles.TMDB_API_KEY -w\n' +
  'You can also skip it: browse the TMDB gallery linked above and paste an ' +
  'image URL.';
...
// Hosts we will fetch a poster from. Narrow on purpose: the editor server takes
// a URL from the browser, and this is the only place it turns into a request.
//
//   image.tmdb.org        — TMDB API paths, and the "open image" link in its UI
//   media.themoviedb.org  — what the TMDB website itself serves thumbnails from,
//                           so a straight copy off the posters gallery lands here
//   a.ltrbxd.com          — Letterboxd's own CDN
const POSTER_HOSTS = new Set(['image.tmdb.org', 'media.themoviedb.org', 'a.ltrbxd.com']);

// TMDB encodes the size in the path (/t/p/w300/abc.jpg). Whatever size got
// copied, normalise to one that is big enough for a poster at 2x render scale
// without pulling a multi-megabyte `original`.
const TMDB_SIZE = 'w780';

export function normalisePosterUrl(src) {
  let u;
  try { u = new URL(String(src).trim()); }
  catch { throw new Error('That is not a URL.'); }

  if (u.protocol !== 'https:') throw new Error('Poster URL must be https.');
  if (!POSTER_HOSTS.has(u.hostname)) {
    throw new Error(
      `Posters can only be fetched from ${[...POSTER_HOSTS].join(', ')} — got ${u.hostname}.\n` +
      'Save the image and drop it into posters/ instead.'
    );
  }

  if (u.hostname !== 'a.ltrbxd.com') {
    // Both TMDB hosts serve the same paths; pin to the API host and one size.
    const m = u.pathname.match(/^\/t\/p\/[^/]+\/(.+)$/);
    if (!m) throw new Error('That TMDB URL is not an image path (/t/p/<size>/<file>).');
    return `https://image.tmdb.org/t/p/${TMDB_SIZE}/${m[1]}`;
  }
  return u.toString();
}
